...
function make_lstm(opt, model)
    ...
    for L = 1,n do
        ...
        if L == 1 then
            local word_vecs
            if model == 'enc' then
                -- word_vecs = nn.Identity()
                word_vecs = nn.Linear(opt.source_size, input_size)
                -- word_vecs = nn.LookupTable(opt.source_size, input_size)
            else
                word_vecs = nn.LookupTable(opt.target_size, input_size)
            end	  
            ...
            if opt.hop_attn == L and model == 'dec' then
                local hop_attn = make_decoder_attn(opt, 1)
...
function make_decoder_attn(opt, simple)
...
function make_generator(opt)
    local model = nn.Sequential()
    model:add(nn.Linear(opt.rnn_size, opt.target_size))
    model:add(nn.LogSoftMax())
    local w = torch.ones(opt.target_size)
    w[1] = 0
